...
describe('auth', () => {
	...
	describe('login', () => {
		it('should successfully login with valid credentials', async () => {
			...
			expect(fetchMock).toHaveBeenCalledWith('http://localhost:1337/api/auth/local', {
				method: 'POST',
	...
	describe('register', () => {
		it('should successfully register with valid data', async () => {
			...
			expect(result).toEqual({ success: true });
			expect(fetchMock).toHaveBeenCalledWith('http://localhost:1337/api/auth/local/register', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({
					username: 'newuser',
	...
	describe('error handling', () => {
		...
		it('should handle network errors in register', async () => {
			...
			await expect(register('newuser', 'new@example.com', 'password123')).rejects.toThrow(
				'Network error'
			);
		});
