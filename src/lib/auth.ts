...
async function fetchConsultantIdByUserId(userId: number, jwt: string): Promise<string | null> {
  const res = await fetch(`${API_URL}/api/consultants?populate=*&filters[user][id][$eq]=${userId}`, {
...
async function fetchConsultantIdByUserIdWithRetry(userId: number, jwt: string, retries = 5, delay = 300) {
