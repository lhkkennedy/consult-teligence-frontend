...
export class UserContentService {
	...
	static async getUserPosts(): Promise<TimelinePost[]> {
		...
		return userPosts.filter(post => post.person_id === user.id?.toString());
	}

	/**
	 * Create a new post for the current user
	 */
	static async createPost(postData: Omit<TimelinePost, 'post_id' | 'person_id' | 'created_at'>): Promise<TimelinePost> {
		if (!user) {
			throw new Error('User not authenticated');
		}
	...
	static async updatePost(postId: string, updates: Partial<TimelinePost>): Promise<TimelinePost> {
		...
		const postIndex = userPosts.findIndex(post => 
			post.post_id === postId && post.person_id === user.id?.toString()
		);
	...
	static async updateProperty(propertyId: string, updates: Partial<Property>): Promise<Property> {
		...
		const propertyIndex = userProperties.findIndex(property => property.property_uid === propertyId);
	...
	static async getPostById(postId: string): Promise<TimelinePost | null> {
		...
		const post = userPosts.find(post => 
			post.post_id === postId && post.person_id === user.id?.toString()
		);
	...
	static async getPortfolioStats(): Promise<{
		...
		const averageIRR = propertiesWithIRR.length > 0 
			? propertiesWithIRR.reduce((sum, prop) => sum + (prop.irr || 0), 0) / propertiesWithIRR.length
			: 0;
	...
	static validatePropertyData(data: any): { isValid: boolean; errors: Record<string, string> } {
		...
		if (data.completion_percentage && (isNaN(Number(data.completion_percentage)) || Number(data.completion_percentage) < 0 || Number(data.completion_percentage) > 100)) {
