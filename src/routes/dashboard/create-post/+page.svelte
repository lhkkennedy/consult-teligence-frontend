<script lang="ts">
	...
	import { ArrowLeft, Image, X, AlertCircle, CheckCircle } from '@lucide/svelte';
	...
	onMount(async () => {
		...
		// Load user's properties for attachment
	...
	async function handleSubmit() {
		...
		try {
			...
			await UserContentService.createPost(formData);

			// Redirect to dashboard
			goto('/dashboard?tab=posts');
		} catch (error) {
			console.error('Error creating post:', error);
			errors.submit = 'Failed to create post. Please try again.';
		} finally {
			submitting = false;
		}
	}
